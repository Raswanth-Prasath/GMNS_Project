...
def osm2gmns_network():
    ...
    input_file = r'D:\ASU Academics\Transportation Modelling and Simulation\TAPLite\python_source_code\asu_map.osm'
    net = og.getNetFromFile(input_file, 
    link_types=(
        'motorway',         # Highways/Freeways
        'trunk',            # Major arterials
        'primary',          # Primary arterials
...
def init_link_pointers(links_file_name,  g_tap_log_file=0, logfile_path=None, use_us_standard = False):
    ...
    if Node == number_of_nodes:
        ...
        for Node in range(Node + 1, number_of_nodes + 1):
            FirstLinkFrom[Node] = 0
            LastLinkFrom[Node] = -1
...
def link_travel_time(k, volume):
    ...
    if k >= len(g_link_vector):
        print(f"Invalid index: {k} exceeds Link size {len(g_link_vector)}")
...
class AlternativeShortestPaths:
    def __init__(self):
        """
        Initialize the class with data structures needed for different shortest path implementations.
        We'll store network data in multiple formats to support different algorithms.
        """
        self.nodes_df = None
        self.links_df = None
        self.graph_nx = None  # NetworkX graph representation
        self.distances = {}   # Dictionary for custom implementations

    def load_network_data(self):
        """
        Loads network data from CSV files and creates necessary graph representations.
        Handles unit conversions and travel time calculations consistently.
        """
        print("Loading and preprocessing network data...")
        
        # Load CSV files
        self.nodes_df = pd.read_csv('node.csv')
        self.links_df = pd.read_csv('link.csv')
        
        # Initialize NetworkX graph for comparison
        ...
        print("Processing links and calculating travel times...")
        for _, link in self.links_df.iterrows():
            # Get link properties and convert units
            length_meters = float(link['length'])
            speed_kmh = float(link.get('free_speed', 60))
            
            # Convert to miles and mph for consistency
            length_miles = length_meters / 1609.34
            speed_mph = speed_kmh / 1.609
            
            # Calculate travel time in minutes
            travel_time = (length_miles / speed_mph) * 60
        ...
        print(f"Network loaded: {len(self.nodes_df)} nodes, {len(self.links_df)} links")

    def dijkstra_with_heap(self, source, target):
        """
        Custom implementation of Dijkstra's algorithm using a binary heap priority queue.
        Optimized for performance with early termination when target is reached.
        """
        distances = {node: float('infinity') for node in self.graph_nx.nodes()}
        distances[source] = 0
        heap = [(0, source)]
        visited = set()
        ...
        while heap:
            ...
            for neighbor in self.graph_nx.neighbors(current_node):
                distance = current_distance + self.graph_nx.edges[current_node, neighbor]['weight']
    ...
    def networkx_shortest_path(self, source, target):
        """
        Calculate shortest path using NetworkX's built-in Dijkstra implementation.
        Includes error handling for disconnected nodes.
        """
        try:
    ...
    def fifo_label_correcting(self, source, target):
        """
        ...
        distances[source] = 0
        queue = deque([source])
        in_queue = {node: False for node in self.graph_nx.nodes()}
        ...
        while queue:
            ...
            for neighbor in self.graph_nx.neighbors(current):
                distance = distances[current] + self.graph_nx.edges[current, neighbor]['weight']
    ...
    def update_accessibility_matrix(self):
        """
        Updates the accessibility matrix with results from all implemented algorithms.
        Includes progress tracking and performance statistics.
        """
        print("\nUpdating accessibility matrix with multiple algorithms...")
        
        # Load current accessibility matrix
        accessibility_df = pd.read_csv('accessibility_matrix.csv')
        
        # Add columns for each algorithm
        accessibility_df['cost_networkx'] = 0.0
        accessibility_df['cost_dijkstra_heap'] = 0.0
        accessibility_df['cost_fifo_lc'] = 0.0
        
        # Process each OD pair
        total_pairs = len(accessibility_df)
        start_time = time.time()
        
        for idx, row in accessibility_df.iterrows():
            if idx % 100 == 0:
                elapsed_time = time.time() - start_time
                pairs_per_second = idx / max(elapsed_time, 1)
                remaining_pairs = total_pairs - idx
                estimated_remaining_time = remaining_pairs / max(pairs_per_second, 0.1)
                
                print(f"Processing OD pair {idx + 1}/{total_pairs} "
                      f"({pairs_per_second:.1f} pairs/sec, "
                      f"~{estimated_remaining_time/60:.1f} minutes remaining)")
            
            o_zone_id = int(row['o_zone_id'])
            d_zone_id = int(row['d_zone_id'])
            
            # Calculate costs using different algorithms
            nx_cost = self.networkx_shortest_path(o_zone_id, d_zone_id)
            dijkstra_cost = self.dijkstra_with_heap(o_zone_id, d_zone_id)
            fifo_cost = self.fifo_label_correcting(o_zone_id, d_zone_id)
            
            # Update the DataFrame
            accessibility_df.loc[idx, 'cost_networkx'] = nx_cost
            accessibility_df.loc[idx, 'cost_dijkstra_heap'] = dijkstra_cost
            accessibility_df.loc[idx, 'cost_fifo_lc'] = fifo_cost
        
        # Save enhanced accessibility matrix
        accessibility_df.to_csv('accessibility_matrix_enhanced.csv', index=False)
        
        # Generate comparison statistics
        print("\nGenerating comparison statistics...")
        stats_df = pd.DataFrame({
            'original_cost': accessibility_df['cost'],
            'networkx_cost': accessibility_df['cost_networkx'],
            'dijkstra_heap_cost': accessibility_df['cost_dijkstra_heap'],
            'fifo_lc_cost': accessibility_df['cost_fifo_lc']
        })
        
        # Calculate relative differences
        for col in ['networkx_cost', 'dijkstra_heap_cost', 'fifo_lc_cost']:
            stats_df[f'{col}_diff_pct'] = (
                (stats_df[col] - stats_df['original_cost']) / 
                stats_df['original_cost'] * 100
            )
        ...
        summary_stats.to_csv('accessibility_algorithm_comparison.csv')
